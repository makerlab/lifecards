...
3) Maintainability and access to your data is critical for you. You want to use your own external tools and your own filesystem as opposed to making yet another shadow copy of your data for yet another content management system.
...
such as lists, cards, maps, globes, and timelines.
We can even expect to be able to manufacture Pinterest-style views, or PowerPoint presentations, or Hypercard style "light programming" interfaces.

Beyond the basics, there is an opportunity to go further. A data-driven view is particularly useful for viewing the same content in many different ways.
In the near future, we will probably see the emergence of heads-up displays and richer viewing metaphors, where it will be common to decorate the real world with virtual data, and where 3D will be common.
...
- 3D information visualization, such as 3D spiral timelines on a year basis.
- Mind palaces made visible, where your data is represented as a Redwood Forest.
- Audio, as in playing a collection as a music stream.
- Obsidian- or mind map-style viewers, for rich, diagrammatic interfaces.

There's also some sensitivity to a class of multiplayer concerns. The app is written to be multiplayer, with every node dynamically updating and injecting changes on the fly, and a database observer pattern is carefully used everywhere. Some uses include:
...
Data is decorated with presentation hints that are used to guide rendering.
Note that of course your 'database' can be your filesystem or however you've chosen to organize your content.

This can mean that in some senses the presentation is less flexible than a full- blown CMS. But the hope is that by reducing complexity, managing larger amounts of data becomes easier.

## Why is it called "Lifecards"?

Throughout this project, we represent a data object as a single card -- a highly relatable pattern in how people collect, organize and share information. They're a useful metaphor (and will be extended upon over time):

- Live cards that reflect, say, the local weather in a region or a stock market price.
- Animated cards for deepwater fish, or plants or birds.
- Cards that can act like a to-do list - letting you mark off each to-do item on completion.
- Cut, paste and remix cards to create new decks; similar to slideshare.
- Print decks out as MooCards and take them with you (such as "Pacific Northwest Birds").

## How about CMS "XYZ"?

Gardening our data is, of course, a perennial problem. Ever since clay tablets appeared in Mesopotamia in 2400 BC, people have been struggling to organize and share information. Scribes in monasteries have shuffled paperwork for eons. Tim Berners-Lee wrote the web as an attempt to try tackle this. Ted Nelson wrote Project Xanadu for similar reasons. Hypercard was developed by Bill Atkinson as yet another response to this perennial challenge.
...
Some of our technical criteria for this project:

- Data-driven. Lifecards is purely data-driven, with all layout based on hints from database objects.
...
- Flat namespace. Rather than having a <code>/public</code> folder I wanted to treat the entire folder space as a network-visible resource.
- Submodules. The intent is to not have a npm_modules or some other crate/module scheme but to include this entire engine as a subfolder for other projects.

And some of our critiques of other systems:
...
- Data has to be poured into them and can vanish at any time.
- Hosting them yourself can require servers and specialized skills to run and manage.
- The tools provided have complex layout capabilities that take real work to master.
...
You should be able to use the current tools as-is, but you can also customize if you wish. It's important to customize, because reader fatigue might set in if the same patterns are seen over and over. Customization can be achieved by including your own CSS, by hand-styling each of your elements, or by adding new kinds of elements even (if you're a programmer). You'll want to familiarize yourself with design theory resources if you start messing around with layout beyond what is provided.

The current version DOES attempt to set a few basics:

- Fonts and font kerning, set to reasonable values
- Default spacing
- Default colors
- A night mode for color
- Narrow mode for mobile support
- A general philosophy of centering content, for simple, quick and satisfying layout wins
...
Look at the Tailwind docs for good examples of higher level layouts and components for the web. Also, there are a number of resources on designing usability for real-world human spaces, for real-world artifacts, for 3D interfaces like video games. These may feel esoteric, but do represent valuable thinking outside of the space of the web and can inform your work.

# Quirks
---

- **Deno.** <br />This project uses [Deno](https://deno.land/) at the moment (see Deno server in <code>/utils/server.js)</code>. The server role is largely to serve http:// files from the current folder. There's no particular reason I'm using Deno; it can be replaced with any server you wish, such as nodejs/express or firebase, or whatever.

- **SPA.** <br />This is a single page app. On the server side I *do* re-direct some path requests back to the root index.html to allow for persistent urls. My strategy is that if a path does not end in a .jpg or some other MIME type then I assume it is a request for SPA route - and I serve the index.html page. Firebase does something similar, for what it's worth.
